...
suffix=""
if [[ "$branch_name" == "develop" || "$branch_name" == "test-linux-arm-publish" ]]; then
  # Develop branch (and throwaway test-linux-arm-publish): use dev versioning with build number
