...
class TestCache:
    ...
    def test_open_passive_reduce_only_orders_for_position_only_returns_current_matching_orders(self):
        # Arrange
        ...
        market_reduce_only_order = self.strategy.order_factory.market(
            AUDUSD_SIM.id,
            OrderSide.SELL,
            Quantity.from_int(100_000),
            reduce_only=True,
        )
        non_reduce_only_order = self.strategy.order_factory.limit(
            AUDUSD_SIM.id,
            OrderSide.SELL,
            Quantity.from_int(100_000),
        ...
        self.cache.add_order(reduce_only_order, position_id)

        # Assert initialized orders are historical only, not open.
        assert self.cache.orders_for_position(position_id) == [reduce_only_order]
        assert self.cache.open_passive_reduce_only_orders_for_position(position_id) == []

        reduce_only_order.apply(TestEventStubs.order_submitted(reduce_only_order))
        self.cache.update_order(reduce_only_order)
        reduce_only_order.apply(
        ...
        self.cache.add_order(market_reduce_only_order, position_id)
        market_reduce_only_order.apply(TestEventStubs.order_submitted(market_reduce_only_order))
        self.cache.update_order(market_reduce_only_order)
        market_reduce_only_order.apply(
            TestEventStubs.order_accepted(
        ...
        self.cache.add_order(non_reduce_only_order, position_id)
        non_reduce_only_order.apply(TestEventStubs.order_submitted(non_reduce_only_order))
        self.cache.update_order(non_reduce_only_order)
        non_reduce_only_order.apply(
            TestEventStubs.order_accepted(
        ...
        # Act, Assert
        assert self.cache.open_passive_reduce_only_orders_for_position(position_id) == [reduce_only_order]

        reduce_only_order.apply(TestEventStubs.order_canceled(reduce_only_order))
        self.cache.update_order(reduce_only_order)

        assert self.cache.open_passive_reduce_only_orders_for_position(position_id) == []

        historical_orders = self.cache.orders_for_position(position_id)
        assert len(historical_orders) == 3
        assert reduce_only_order in historical_orders
        assert market_reduce_only_order in historical_orders
        assert non_reduce_only_order in historical_orders
